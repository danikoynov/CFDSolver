...
def impose_boundary_conditions(
    ...
    def is_inside_airfoil(i: int, j: int) -> bool:
        ...
        x_local = x_pivot + xr * math.cos(alpha) + yr * math.sin(alpha)
        y_local = y_pivot - xr * math.sin(alpha) + yr * math.cos(alpha)

        X = (x_local - x_le) / airfoil.chord
        ...
        return (y_airfoil_center - y_t) <= y_local <= (y_airfoil_center + y_t)
    ...
    print(f"Angle of attack: {aoa_deg} degrees")
...
if __name__ == "__main__":
    ...
    impose_boundary_conditions(sim, airfoil, v_air, dx, aoa_deg)
