...
void test_type_out_of_range_returns_boundary() {
    ...
    require(
        bc.type(-1, 0) == CellType::BOUNDARY,
        "type(-1,0) should be BOUNDARY"
    );
    require(
        bc.type(4, 0) == CellType::BOUNDARY,
        "type(4,0) should be BOUNDARY"
    );
    require(
        bc.type(0, -1) == CellType::BOUNDARY,
        "type(0,-1) should be BOUNDARY"
    );
    require(
        bc.type(0, 3) == CellType::BOUNDARY,
        "type(0,3) should be BOUNDARY"
    );
}
...
void test_prescribe_u_value_inserts() {
    ...
    const std::size_t id =
        static_cast<std::size_t>(2) * 3 + static_cast<std::size_t>(1);
...
void test_prescribe_v_value_inserts() {
    ...
    const std::size_t id =
        static_cast<std::size_t>(2) * (3 + 1) + static_cast<std::size_t>(1);
...
void test_prescribe_p_too_negative_i_throws() {
    BoundaryConditions bc(4, 3);

    bool threw = false;
    try {
        bc.prescribe_p_value(-2, 0, 1.0);
    } catch (const std::out_of_range&) {
        threw = true;
    }

    require(threw, "test_prescribe_p_too_negative_i_throws failed");
}

void test_prescribe_p_too_negative_j_throws() {
    BoundaryConditions bc(4, 3);

    bool threw = false;
    try {
        bc.prescribe_p_value(0, -2, 1.0);
    } catch (const std::out_of_range&) {
        threw = true;
    }

    require(threw, "test_prescribe_p_too_negative_j_throws failed");
}
...
void test_prescribe_p_accepts_extended_boundary_indices() {
    BoundaryConditions bc(4, 3);

    bc.prescribe_p_value(-1, 0, 1.0);
    bc.prescribe_p_value(4, 3, 2.0);

    require(
        bc.prescribed_p().size() == 2,
        "test_prescribe_p_accepts_extended_boundary_indices: wrong map size"
    );
}
...
void test_prescribed_p_boundary_getter_returns_value() {
    BoundaryConditions bc(4, 3);

    bc.prescribe_p_value(-1, 2, 8.0);

    require(
        bc.prescribed_p(-1, 2) == 8.0,
        "test_prescribed_p_boundary_getter_returns_value failed"
    );
}
...
void test_prescribed_p_missing_value_returns_zero() {
    BoundaryConditions bc(4, 3);

    require(
        bc.prescribed_p(1, 1) == 0.0,
        "test_prescribed_p_missing_value_returns_zero failed"
    );
}
...
void test_is_u_prescribed_true_false() {
    ...
    require(
        bc.is_u_prescribed(2, 1),
        "test_is_u_prescribed_true_false: expected true"
    );
    require(
        !bc.is_u_prescribed(1, 1),
        "test_is_u_prescribed_true_false: expected false"
    );
}
...
void test_is_v_prescribed_true_false() {
    ...
    require(
        bc.is_v_prescribed(2, 1),
        "test_is_v_prescribed_true_false: expected true"
    );
    require(
        !bc.is_v_prescribed(1, 1),
        "test_is_v_prescribed_true_false: expected false"
    );
}
...
void test_is_p_prescribed_true_false() {
    ...
    require(
        bc.is_p_prescribed(2, 1),
        "test_is_p_prescribed_true_false: expected true"
    );
    require(
        !bc.is_p_prescribed(1, 1),
        "test_is_p_prescribed_true_false: expected false"
    );
}
...
int main() {
    ...
    run(
        test_prescribe_u_same_value_duplicate_keeps_size,
        "prescribe u same value duplicate keeps size"
    );
    run(
        test_prescribe_v_same_value_duplicate_keeps_size,
        "prescribe v same value duplicate keeps size"
    );
    run(
        test_prescribe_p_same_value_duplicate_keeps_size,
        "prescribe p same value duplicate keeps size"
    );
    ...
    run(test_prescribe_p_too_negative_i_throws, "prescribe p too negative i throws");
    run(test_prescribe_p_too_negative_j_throws, "prescribe p too negative j throws");
    ...
    run(
        test_prescribe_p_accepts_extended_boundary_indices,
        "prescribe p accepts extended boundary indices"
    );
    ...
    run(
        test_prescribed_p_boundary_getter_returns_value,
        "prescribed p boundary getter returns value"
    );
    ...
    run(
        test_prescribed_p_missing_value_returns_zero,
        "prescribed p missing value returns default pressure"
    );
    ...
    run(
        test_is_u_prescribed_out_of_range_throws,
        "is u prescribed out of range throws"
    );
    run(
        test_is_v_prescribed_out_of_range_throws,
        "is v prescribed out of range throws"
    );
    run(
        test_is_p_prescribed_out_of_range_throws,
        "is p prescribed out of range throws"
    );
